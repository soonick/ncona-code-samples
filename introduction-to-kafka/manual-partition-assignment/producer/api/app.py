...
def produce(message):
    partition = hash(message['name']) % 2
    message_str = json.dumps(message)
    app.logger.info(f'Writing message: "{message_str}" to topic: {PRODUCTS_TOPIC} and partition {partition}')
    producer.produce(PRODUCTS_TOPIC, value=message_str, callback=produce_callback, partition=partition)
    producer.flush()
